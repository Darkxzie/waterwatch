...
export default function Analytics() {
  ...
  const byType = (summary?.byType || []).map((item) => ({
    name: item.issueType,
    total: item._count._all,
  }));
  const bySeverity = (summary?.bySeverity || []).map((item) => ({
    name: item.aiSeverity || 'UNKNOWN',
    value: item._count._all,
  }));
  const trendData = Object.values(
    trends.reduce((accumulator, complaint) => {
      const day = new Date(complaint.createdAt).toLocaleDateString('en-IN', {
        month: 'short',
        day: 'numeric',
      });
  ...
  const resolutionByType = Object.values(
    resolution.reduce((accumulator, item) => {
      const durationHours =
        (new Date(item.resolvedAt) - new Date(item.createdAt)) / (1000 * 60 * 60);
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm leading-7 text-mist">
            A darker, more legible operations surface for trend monitoring and response analysis.
          </p>
        </div>
      </div>
      {summaryError ? (
        <div className="glass-panel rounded-[2rem] p-6 text-critical">
          Login as the seeded admin to view analytics.
        </div>
      ) : null}
