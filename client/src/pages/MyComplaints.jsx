...
export default function MyComplaints() {
  ...
  return (
    <PageWrapper className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm leading-7 text-mist">
            Track issue state, AI summary, and response progress in a calmer dark interface.
          </p>
        </div>
      ...
      {isLoading ? (
        <div className="glass-panel rounded-[2rem] p-6 text-mist">Loading your complaints...</div>
      ) : null}
      {error ? (
      ...
      {!isLoading && !error && complaints.length === 0 ? (
        <div className="glass-panel rounded-[2rem] p-6 text-mist">
          No complaints yet. Report your first issue from the Report page.
        </div>
      ) : null}
